...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		/* Server end do not have this file */
		if(strncmp(file, "open: No such file or directory", 31) == 0){
			printf("Server[%s]: No such file or directory\n", filename);
			if(unlink(filename) < 0){/* Delete file */
				perror("filename");
				exit(EXIT_FAILURE);
			}
			break;/* Exit loop */
