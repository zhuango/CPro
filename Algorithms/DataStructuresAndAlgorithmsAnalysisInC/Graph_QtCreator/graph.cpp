...
void djikstraTest()
{
	...
	TableInstance t;
    InintTable(mapping["v1"], graph, t, VertexNumber);
	Dijkstra(graph, mapping["v1"], t);
    for(UInt32 i = 1; i <= graph->Size; i ++)
	{
...
{
    Graph *graph = InitializeGraph();
    TableInstance t = new Table[graph->Cells.size()];
    InintTable(mapping["v1"], graph, t, VertexNumber);
...
{
    Graph *graph = InitializeGraph();
    TableInstance t = new Table[graph->EdgeNumber + 1];
    InitializeTableForKruskal(graph, t);
    list<Edge> edges;
    Kruskal(graph, t, edges);

    list<Edge>::iterator iter = edges.begin();
    for(;iter != edges.end(); iter ++)
    {
        cout << "<" << (*iter).Node1 << ", " << (*iter).Node2 << ", " << (*iter).weight << ">" << endl;
    }
    DestroyGraph(graph);
    delete[] t;
    return 0;
